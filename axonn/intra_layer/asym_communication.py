...
def _allgatherv(tensor, rank_local_batch_sizes, process_group=None):
    ...
    for batch_size in rank_local_batch_sizes:
        ...
        output_tensor_list.append(torch.empty(tuple(shape), device=tensor.device, dtype=tensor.dtype))
...
def _gather_batch_scatter_channels(input_, rank_local_batch_sizes, process_group=None):
    ...
    for i in range(world_size):
        ...
        recv_tensors.append(torch.empty(tuple(shape), device='cuda', dtype=input_.dtype))
...
def _gather_channels_scatter_batch(input_, rank_local_batch_sizes, process_group=None):
    ...
    # then this returns a tensor of [m_{i},....,k], where m_{i} = rank_local_batch_sizes[i] 
    input_ = input_.contiguous()
    world_size = torch.distributed.get_world_size(process_group)
...
class GatherBatchScatterChannels(torch.autograd.Function):
    ...
    @staticmethod
    def symbolic(graph, input_, rank_local_batch_sizes, process_group=None):
        output = _gather_batch_scatter_channels(input_, rank_local_batch_sizes, process_group)
        graph.process_group = process_group
        graph.rank_local_batch_sizes = rank_local_batch_sizes
        return output
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = _gather_channels_scatter_batch(grad_output, rank_local_batch_sizes, ctx.process_group)
...
class GatherChannelsScatterBatch(torch.autograd.Function):
    ...
    @staticmethod
    def symbolic(graph, input_, rank_local_batch_sizes, process_group=None):
        output = _gather_channels_scatter_batch(input_, rank_local_batch_sizes, process_group)
        graph.process_group = process_group
        graph.rank_local_batch_sizes = rank_local_batch_sizes
        return output
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = _gather_batch_scatter_channels(grad_output, rank_local_batch_sizes, ctx.process_group)
...
if __name__ == "__main__":
    ...
    tensor = torch.randn((dist.get_rank()+5, 8), device="cuda", dtype=torch.bfloat16, requires_grad=True)
