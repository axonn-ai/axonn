...
class communication_handle:
    ...
    def __init__(
        self, G_inter: int, G_data: int, G_intra_r=1, G_intra_c=1, G_intra_d=1, gpus_per_node=None
    ):
        ...
        self.intra_layer_row_parallel_rank = (self.intra_layer_parallel_rank // G_intra_c) % G_intra_r
        self.intra_layer_depth_parallel_rank = (self.intra_layer_parallel_rank // (G_intra_c * G_intra_r) )
        ...
        for i in range(G_data):
            for j in range(G_inter):
                ...
                assert len(ranks_in_ith_jth_intra_layer_group) == G_intra_r * G_intra_c * G_intra_d
                
                ranks_in_ith_jth_intra_layer_group = np.array(ranks_in_ith_jth_intra_layer_group).reshape(G_intra_d, G_intra_r, G_intra_c)
                ...
                for i in range(G_intra_d):
                    for j in range(G_intra_r):
                        group_members = list(ranks_in_ith_jth_intra_layer_group[i, j, :])
