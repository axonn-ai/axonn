...
class CPUAdam(Optimizer):
    ...
    def __init__(
        ...
        assert (
            ax.computation_dtype == torch.bfloat16
            or ax.computation_dtype == torch.float16
        )
        assert ax._float16_all_reduce
    ...
    def step(self, closure=None):
        ...
        flat_float16_grad = ax.model_grads_float16
        flat_float16_params = ax.model_params_float16
        ...
        for group_no in range(len(self.group_offsets) - 1):
            ...
            while start_index != self.group_offsets[group_no + 1]:
                ...
                event = ax.comm_handle.allreduce(
                    flat_float16_grad[start_index:end_index], async_op=True
                )
            ...
            while start_index != self.group_offsets[group_no + 1]:
                ...
                with torch.cuda.stream(stream):
                    ...
                    grad_buffer[:size].copy_(
                        flat_float16_grad[start_index:end_index], non_blocking=True
                    )
                    ...
                    flat_float16_params[start_index:end_index].copy_(
