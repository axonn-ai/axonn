...
from ptb_loader import ptb_dataset, get_collate_fn, init_vocab


dataset = ptb_dataset('./examples/dataset/PTB/', seq_length=128, word2ind=init_vocab('./examples/dataset/PTB'))
bs_per_gpu = 64
num_gpus = 6
...
model = DistributedGPT(N, D, H, vocab_size=vocab_size, seq_len=seq_len, ckp_coeff=8).cuda()
...
for epoch_number in range(num_epochs):
    ...
    for sent in tqdm(
        ...
        optimizer.step()
        if not log_memory:
            ax.print_status(f'With cpu_offload = {cpu_offload}, '
                            f'Current memory utilisation = {torch.cuda.memory_allocated() /1e9} GB, '
                            f'Max memory utilisation = {torch.cuda.max_memory_allocated() /1e9} GB, ')
            log_memory = True
