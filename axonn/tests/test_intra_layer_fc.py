...
def test_fw_pass(G_intra_r, G_intra_c, B, H):
    ...
    X_local = _drop(X, 1, inner_group)  # divide colunns of X along the inner tensor group
    ...
    with torch.no_grad():
        ...
        weight_sequential = _gather(_gather(layer.linear.weight, 1, inner_group), 0, outer_group)
...
def test_bw_pass(G_intra_r, G_intra_c, B, H):
    ...
    layer = Tensor_Parallel_Linear(in_features=H, out_features=H, bias=False).cuda()
    X_local = _drop(X, 1, inner_group).detach().clone()  # divide colunns of X along the inner tensor group
    ...
    assert torch.allclose(X_grad_parallel, X.grad), "BW Pass - gradients of input do not match"

    weight_grad_parallel =  _gather(_gather(layer.linear.weight.grad, 1, inner_group), 0, outer_group)
    assert torch.allclose(weight_grad_parallel, layer_sequential.weight.grad), "BW Pass - gradients of weight do not match"
